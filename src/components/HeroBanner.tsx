...
export function HeroBanner({ champions }: { champions: Champion[] }) {
  ...
  return (
    <section className="relative overflow-hidden border-b border-white/10">
      <div className="absolute inset-0 -z-10 bg-[linear-gradient(135deg,rgba(66,214,255,0.15),transparent_30%),linear-gradient(45deg,rgba(255,107,61,0.11),transparent_34%),radial-gradient(circle_at_50%_20%,rgba(184,255,75,0.08),transparent_30%)]" />
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.08fr_0.92fr] lg:px-8 lg:py-20">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-xl font-semibold leading-8 text-slate-300">
          ...
          <div className="mt-8 max-w-2xl">
            <label className="relative block">
              ...
              <input
                ...
                className="h-14 w-full rounded-md border border-white/[0.12] bg-abyss/[0.72] pl-12 pr-4 text-base font-bold text-white shadow-card outline-none transition placeholder:text-slate-500 focus:border-frost/55 focus:ring-4 focus:ring-frost/10"
              />
            </label>
            <div className="mt-3 grid gap-2 sm:grid-cols-5">
              {matches.map((champion) => (
                <Link
                  ...
                  className="rounded-md border border-white/10 bg-white/[0.055] px-3 py-2 text-center text-xs font-black text-slate-200 transition hover:border-frost/45 hover:text-white"
                >
                  {champion.name}
                </Link>
              ))}
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href="/champions"
              className="inline-flex items-center gap-2 rounded-md bg-frost px-5 py-3 text-sm font-black text-abyss transition hover:bg-volt"
            >
              Explore Champions
            ...
            <Link
              href="/broken-builds"
              className="inline-flex items-center gap-2 rounded-md border border-ember/40 bg-ember/10 px-5 py-3 text-sm font-black text-ember transition hover:bg-ember/20 hover:text-white"
            >
        ...
        <div className="premium-border relative min-h-[28rem] overflow-hidden rounded-lg bg-panel/[0.68] p-5 shadow-card">
            ...
            <div className="mt-8 space-y-3">
              {champions.slice(0, 5).map((champion, index) => (
                <Link
                  key={champion.slug}
                  href={`/champions/${champion.slug}`}
                  className="grid grid-cols-[2.2rem_1fr_auto] items-center gap-3 rounded-md border border-white/10 bg-abyss/[0.58] p-3 transition hover:border-frost/40 hover:bg-white/[0.065]"
                >
                  ...
                  <span>
                    ...
                    <span className="text-xs font-bold text-slate-500">{champion.arenaStats.brokenBuild.name}</span>
                  </span>
